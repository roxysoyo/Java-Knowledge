...
| 特性                   | 规则说明                                                     |
| :--------------------- | :----------------------------------------------------------- |
| **1. 节点颜色**        | 每个节点是**红色**或**黑色**                                 |
| **2. 根节点颜色**      | 根节点必须是**黑色**                                         |
| **3. 叶子节点（NIL）** | 所有叶子节点（NIL / 空节点）是**黑色**                       |
| **4. 红色节点限制**    | 红色节点的**两个子节点必须是黑色**（即不能有连续的红色节点） |
| **5. 黑色高度一致**    | 从任一节点到其每个叶子节点的所有路径，都包含**相同数量的黑色节点** |
| **6. 新增节点颜色**    | 新插入的节点默认为**红色**（除非是根节点）                   |
| **7. 平衡性**          | 通过**变色**和**旋转**（左旋、右旋）维持上述特性             |
